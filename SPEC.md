...
| `presentSkip` | boolean | no | Left out of presentation mode's steps; stays visible throughout. For a legend, a frame, a heading. |
...
| `edgeStyle` | string | no | `straight`, `bezier` or `orthogonal` (right angles). Absent follows the viewer's preference. |
...
| `routed` | boolean | no | The `waypoints` were computed by the editor's router, not placed by hand. A routed edge may be recomputed when shapes move; a hand-shaped one is only rebuilt when its route is actually broken. Absent means hand-made. |
...
- **2 MB** per document. Larger gets `413`. An account on a paid plan may
  publish up to **8 MB** — paid plans are not on sale yet, so every account is
...
- **Anonymous links expire 90 days after creation** and then return `404`. Sign
  ...
  link permanent and to be able to list or revoke it later — an agent does that
  with an API token, below.

### Publishing into an account

A publish that carries no credential is anonymous, and an anonymous link has no
owner: it cannot be listed, cannot be revoked, and dies after 90 days. An
account on a paid plan can mint an **API token** (Your account → API tokens) and
give it to an agent:
...
The link then belongs to that account: it appears under *My shared links*, it
can be renamed, protected or revoked, and it does not expire on its own. The
same token works on the account's other endpoints — listing and revoking links,
`GET /api/auth/me` — but **not** on the routes that could take the account over
(deleting it, moving its handle, minting further tokens); those are
browser-session only.

There is an MCP server over these endpoints — `npx -y drillflow-mcp`, free and
MIT ([source](https://github.com/capydev42/drillflow-mcp)) — which publishes a
document or a Mermaid flowchart and reads a published diagram back. It takes the
token in `DRILLFLOW_TOKEN`, and says in every result whether the link it made is
owned or anonymous.

An `Authorization` header that names no live token is a **401**, on every route
including `POST /api/doc`. A revoked or expired token therefore fails loudly
rather than quietly publishing an anonymous link the caller believes it owns.
Tokens are stored as a SHA-256 hash and shown exactly once, when they are
created.

`GET /api/doc/{slug}` returns `{"data": ..., "allowCopy": true}` for a live
link, and `404` for one that is missing, revoked or expired. `allowCopy: false`
is the owner asking that the diagram not be taken away as a document: the viewer
then offers no copy and no document-shaped export (JSON, Mermaid, Markdown, an
image with the scene embedded), only pictures. It is a stated intention, not a
protection — the document is in the response you just read.

An account-owned link on a paid plan may also carry a readable address of the
form
`/view/@handle/name`, read with `GET /api/doc/@{handle}/{name}`. That response
adds the canonical `slug`, which is what `unlock` and the preview image are
addressed by — the plain `/view/{slug}` URL keeps working, since a name is a
second way in rather than a move.

A link whose owner gave it a password answers `401` with
`{"detail": ..., "protected": true}` — the link exists, and the document is
handed over by `POST /api/doc/{slug}/unlock` with `{"password": "..."}`, which
returns the same body as the `GET` (or `403` for the wrong password). Such a
link also has no preview image: `GET /api/doc/{slug}/preview.png` answers `404`
while a password is set, so a chat client cannot unfurl past it.
