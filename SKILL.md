...
- Omit `anchorDeg` — edges float and re-attach themselves correctly. That is
  what you want. (`anchorRel` exists for one thing only: pointing at a spot
  *inside* an `image`. You will not need it either.)
...
- **Never write `portOf` or `showPorts` yourself.** A node with `portOf` is a
  *boundary port*: the editor puts it inside a subflow to stand for one edge on
  the level above, so an arrow inside can dock to what crosses the boundary. If
  you are editing a document that already has them, copy them through untouched,
  including their size and position — and leave them out of any summary you
  write, they are the boundary showing through, not steps somebody drew.
...
list — the editor refuses the whole document on the first failure. (It does
complete a few omissions rather than refuse them — an edge's `id`, a node's
`type` and `label`, the titles — and `POST /api/doc` returns them in a
`warnings` array. Write them anyway: the ones below are not on that list.)
...
3. Every edge's `from.nodeId` and `to.nodeId` exist **in that same diagram**.
   (An endpoint naming a node that is not there is dropped with the edge rather
   than refused — but a diagram missing the arrows you meant to draw is still a
   diagram you got wrong.)
...
If you are running with tools rather than a shell, there is an MCP server for
exactly this — `npx -y drillflow-mcp` (free, MIT), with `publish_diagram`,
`publish_mermaid` and `read_diagram`. It carries the same rules as below.

Say these out loud to the user rather than burying them:

- **Anonymous links expire 90 days after creation.** Signing in at
  app.drillflow.de before sharing makes a link permanent and revocable. An
  account on a paid plan can mint an API token there and publish with
  `-H 'Authorization: Bearer dfp_…'` — the link then belongs to that account,
  listable and revocable, with no expiry. A token that has been revoked or has
  expired answers 401; it never falls back to an anonymous link.
- The link is **public to anyone holding it**. Do not publish anything
  confidential without asking first.
- Limits: 10 publishes per minute per IP, 2 MB per document (8 MB on a paid
  plan; paid plans are not on sale yet).
